# RPV_squark300_cascade_LSP250

**AIDA-Scout S5 (stealth SUSY / light RPV cascade)** -- generated by `make_signal_cards.py`.

Pair production of 300 GeV right-handed up squarks, each decaying to a quark plus a 250 GeV neutralino LSP, with the LSP decaying to three light quarks through the lambda''_112 UDD operator. Eight partons at the hard-process level, no MET, no isotropy -- the 'high multiplicity but not isotropic' corner.
...
apptainer exec --bind $simdir \
  ...
  sh $simdir/run.sh RPV_squark300_cascade_LSP250 <nevents> <seed> $simdir False
```
