...
Pair production of degenerate light squarks at 300 GeV, each decaying through the R-parity-violating lambda'' UDD operator directly into two light quarks. The result is a high-multiplicity all-hadronic event with **no MET** -- a different menu failure mode from S1: high multiplicity but *not* isotropic.
...
* m_squark = 300 GeV; stops, sbottoms, the left-handed light-flavour squarks, the gluino and all gauginos decoupled at 500 GeV, so squark pair production dominates and no MET is produced
...
|---|---|---|
| PuppiHT | 450 (constituents pT > 30) | **yes, 30.6%** |
| SinglePuppiJet | 230 | **yes, 20.0%** |
...
| PuppiETmiss | 200 | no -- 2.6% over 200 GeV, and that tail is jet resolution at PU 200, not a real LSP |
...
The sample is now the four right-handed light-flavour squarks, u_R, c_R, d_R and s_R, degenerate at 300 GeV, with everything else at 500 GeV.
...
apptainer exec --bind $simdir \
  ...
  sh $simdir/run.sh RPV_squark300_UDD <nevents> <seed> $simdir False
```
