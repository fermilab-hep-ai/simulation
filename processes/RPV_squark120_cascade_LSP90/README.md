# RPV_squark120_cascade_LSP90

**AIDA-Scout S5 (stealth SUSY / light RPV cascade)** -- generated by `make_signal_cards.py`.

Pair production of 120 GeV right-handed up squarks, each decaying to a quark plus a 90 GeV neutralino LSP, with the LSP decaying to three light quarks through the lambda''_112 UDD operator. Eight partons at the hard-process level, no MET, no isotropy -- the 'high multiplicity but not isotropic' corner.
...
* m_squark = 120 GeV, m_LSP = 90 GeV, splitting 30 GeV
* Only lambda''_112 is switched on, so the decays are to light quarks
* Gluino, third generation and the heavier gauginos decoupled at 520 GeV
...
|---|---|---|
| PuppiHT | 450 (constituents pT > 30) | no (HT30 median 356 GeV, 4 events) |
| SinglePuppiJet | 230 | no (leading jet median 165 GeV, 4 events) |
...
Shape measured on **only 4 unweighted events** -- indicative at best:
...
apptainer exec --bind $simdir \
  ...
  sh $simdir/run.sh RPV_squark120_cascade_LSP90 <nevents> <seed> $simdir False
```
