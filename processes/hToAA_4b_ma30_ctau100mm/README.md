# hToAA_4b_ma30_ctau100mm

**AIDA-Scout S4 (h -> a a, a -> b bbar)** -- generated by `make_signal_cards.py`.

A 125 GeV Higgs produced by gluon fusion decays to a pair of light pseudoscalars of mass 30 GeV, each decaying to b bbar. The final state is four b bbar objects sharing 125 GeV, so each carries roughly 31 GeV before any further decay.

The pseudoscalar is given a proper lifetime of 100 mm, so the four objects emerge from two displaced vertices. The prompt reference is `hToAA_4b_ma30`.

## Configuration

* Hard process: `HiggsSM:gg2H`, m_h = 125 GeV
* `25:addChannel -> 36 36`, all SM Higgs decays closed
* m_a = 30 GeV, `36:addChannel -> 5 -5`, **ctau = 100 mm**
...
apptainer exec --bind $simdir \
  ...
  sh $simdir/run.sh hToAA_4b_ma30_ctau100mm <nevents> <seed> $simdir False
```
